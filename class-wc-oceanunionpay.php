...
/**
 * Oceanpayment Unionpay Payment Gateway
 *
 * Provides a Oceanpayment Unionpay Payment Gateway, mainly for testing purposes.
 *
 * @class 		WC_Gateway_Unionpay
 * @extends		WC_Payment_Gateway
 * @version		1.0
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Oceanunionpay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'oceanunionpay';
        $this->icon               = apply_filters('woocommerce_oceanunionpay_icon', plugins_url( 'images/op_Unionpay.jpg', __FILE__ ));
        $this->has_fields         = false;
        $this->method_title       = __( 'Oceanpayment UnionPay', 'woocommerce' );
        ...
        // Actions
        add_action( 'woocommerce_api_wc_gateway_oceanunionpay', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-oceanunionpay-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_oceanunionpay', array( $this, 'thankyou_page' ) );
        ...
        add_action( 'woocommerce_receipt_oceanunionpay', array( $this, 'receipt_page' ) );
    }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable Oceanpayment UnionPay', 'woocommerce' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'UnionPay', 'woocommerce' ),
                'desc_tip'    => true,
            ),
            'description' => array(
    ...
    public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
        if ( $this->instructions && ! $sent_to_admin && 'oceancreditcard' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
    ...
    function receipt_page( $order ) {
        echo $this->generate_unionpay_form( $order );
    }
     ...
     * 生成 Unionpay form.
     */
    public function generate_unionpay_form( $order_id ) {
        ...
        //账单人州(可不提交)
        $billing_state     = $this->get_unionpay_state( $order->get_billing_country(), $order->get_billing_state() );
        //账单人城市
        ...
        //收货人的州（省、郡）
        $ship_state 	   = $this->get_unionpay_state( $order->get_shipping_country(), $order->get_shipping_state() );
        //收货人的城市
        ...
        $data_to_send .= "</div>";
        $data_to_send .= "</div>";
        $data_to_send .= "<form  method='post' name='unionpay_checkout' action='".$this->settings['submiturl']."'  >";
        ...
        if($this->settings['mode'] == 'redirect'){
            ...
            $data_to_send .= 'document.unionpay_checkout.submit();' . "\n";
            $data_to_send .= '</script>' . "\n";

        }elseif($this->settings['mode'] == 'iframe'){
            $data_to_send .= '<iframe width="100%" height="700px"  scrolling="auto" style="border:none ; margin: 0 auto; overflow:hidden;" id="ifrm_unionpay_checkout" name="ifrm_unionpay_checkout"></iframe>' . "\n";

            $data_to_send .= '<script type="text/javascript">' . "\n";
            $data_to_send .= 'document.unionpay_checkout.target="ifrm_unionpay_checkout";' . "\n";
            $data_to_send .= 'document.unionpay_checkout.submit();' . "\n";
            $data_to_send .= 'var ifrm_cc  = document.getElementById("ifrm_unionpay_checkout");' . "\n";
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-unionpay-gateway' ) );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())) {
                            $order->update_status('on-hold', __($testorder . $return_info['payment_details'], 'oceanpayment-unionpay-gateway'));
                        }
                        ...
                        if($errorCode == '20061'){
                            ...
                            if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())) {
                                $order->update_status('failed', __($testorder . $return_info['payment_details'], 'oceanpayment-unionpay-gateway'));
                            }
                        }
    ...
    function return_payment( $order ) {
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                //成功
                $order->update_status( 'processing', __( $testorder.$payment_details, 'oceanpayment-unionpay-gateway' ) );
                WC()->cart->empty_cart();
                ...
                if(empty($this->completed_orders()) || !in_array($order_number, $this->completed_orders())) {
                    $order->update_status('on-hold', __($testorder . $payment_details, 'oceanpayment-unionpay-gateway'));
                }
                $url = $this->get_return_url( $order );
                wc_add_notice( $testorder.$payment_details, 'success' );
                ...
                if($errorCode == '20061'){
                    ...
                    if(empty($this->completed_orders()) || !in_array($order_number, $this->completed_orders())) {
                        $order->update_status('failed', __($testorder . $payment_details, 'oceanpayment-unionpay-gateway'));
                    }
     ...
     * 获取州/省
     */
    public function get_unionpay_state( $cc, $state ) {
